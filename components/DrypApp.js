...
const Empty=({text,action,onAction})=><div style={{textAlign:"center",padding:50,color:T.dim}}><div style={{fontSize:13,marginBottom:14}}>{text}</div>{action&&<Btn primary onClick={onAction}>+ {action}</Btn>}</div>
...
export default function DrypApp({data,update,save,user,onLogout}){
  ...
  const nav=[{id:"dashboard",l:"Overblik",i:"◻"},{id:"production",l:"Produktion",i:"⬡"},{id:"recipes",l:"Opskrifter",i:"◉"},{id:"haccp",l:"HACCP Logs",i:"✓"},{id:"batches",l:"Batches",i:"⬢"},{id:"customers",l:"Kunder & Ordrer",i:"◎"},{id:"inventory",l:"Lager",i:"▦"},{id:"planning",l:"Indkøbsplan",i:"◇"},{id:"economy",l:"Økonomi",i:"◈"},{id:"team",l:"Team",i:"☰"}]
  const Pg={dashboard:Dashboard,production:Production,recipes:Recipes,haccp:HACCPLogs,batches:Batches,customers:Customers,inventory:Inventory,planning:Planning,economy:Economy,team:Team}[page]||Dashboard
  ...
  return<div style={{display:"flex",height:"100vh",overflow:"hidden"}}>
    ...
    <div style={{flex:1,display:"flex",flexDirection:"column",overflow:"hidden"}}>
      ...
      <main style={{flex:1,overflow:"auto",padding:18}} className="fade-in" key={page}><Pg data={data} update={update} save={save}/></main>
    </div>
  </div>
}
...
function Recipes({data,update}){
  ...
  return<div style={{maxWidth:900}}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:20}}><div style={{fontSize:11,color:T.dim}}>Produktopskrifter med stykliste og procestrin</div><Btn primary onClick={newR}>+ Ny opskrift</Btn></div>
    ...
    {show&&<Modal title={form.name||"Ny opskrift"} onClose={()=>setShow(false)} wide>
      ...
      <div style={{borderTop:`1px solid ${T.brdL}`,marginTop:8,paddingTop:14}}>
        <div style={{display:"flex",justifyContent:"space-between",marginBottom:10}}><span style={{fontSize:11,fontWeight:700,color:T.acc,textTransform:"uppercase"}}>Stykliste (BOM)</span><Btn small onClick={()=>setForm({...form,bom:[...form.bom,{itemId:data.inventory[0]?.id||"",qty:1,unit:data.inventory[0]?.unit||"stk"}]})}>+ Materiale</Btn></div>
      ...
      <div style={{borderTop:`1px solid ${T.brdL}`,marginTop:14,paddingTop:14}}>
        <div style={{display:"flex",justifyContent:"space-between",marginBottom:10}}><span style={{fontSize:11,fontWeight:700,color:T.acc,textTransform:"uppercase"}}>Procestrin</span><Btn small onClick={()=>setForm({...form,steps:[...(form.steps||[]),""]})}>+ Trin</Btn></div>
...
function Production({data,update}){
  ...
  return<div style={{maxWidth:920}}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:18}}><div style={{fontSize:11,color:T.dim}}>Produktionslog med HACCP CCP1+CCP2</div><Btn primary onClick={startNew}>+ Ny produktion</Btn></div>
...
function HACCPLogs({data,update}){
  ...
  return<div style={{maxWidth:920}}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}><div style={{fontSize:11,color:T.dim}}>HACCP egenkontrol-logs</div><Btn primary onClick={newE}>+ Ny log</Btn></div>
...
function Planning({data}){
  ...
  return<div style={{maxWidth:1000}}>
    <Card style={{marginBottom:20}}><div style={{fontSize:12,fontWeight:600,marginBottom:12}}>Planlagt produktion</div><div style={{display:"flex",gap:14,flexWrap:"wrap"}}>{recipes.map(r=><div key={r.id} style={{flex:"1 1 200px"}}><div style={{fontSize:11,color:T.mid,marginBottom:4}}>{r.name}</div><div style={{display:"flex",alignItems:"center",gap:8}}><input type="number" value={planQty[r.id]||0} onChange={e=>setPlanQty({...planQty,[r.id]:e.target.value})} style={{width:80}}/><span style={{fontSize:11,color:T.dim}}>flasker</span></div></div>)}</div></Card>
    <div style={{fontSize:12,fontWeight:600,marginBottom:12}}>Indkøbsbehov</div>
    {plan.map(item=><Card key={item.id} style={{marginBottom:8,padding:12,borderLeft:item.needsOrder?`3px solid ${T.red}`:`3px solid ${T.ok}`}}><div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}><div><div style={{fontSize:13,fontWeight:500}}>{item.name}</div><div style={{fontSize:10,color:T.dim}}>Behov: {item.need.toFixed(1)} {item.unit} · Lager: {item.qty} · Lead: {item.leadDays}d{item.supplier&&` · ${item.supplier}`}</div></div><div style={{textAlign:"right"}}>{item.needsOrder?<><div style={{fontSize:16,fontWeight:700,fontFamily:T.fm,color:T.red}}>{item.orderQty} {item.unit}</div><div style={{fontSize:10,color:T.warn}}>Bestil senest {item.orderBy}</div><div style={{fontSize:10,color:T.dim}}>~{fk(Math.round(item.orderQty*(item.costPer||0)))}</div></>:<Badge c={T.ok}>OK</Badge>}</div></div></Card>)}
...
function Inventory({data,update}){
  ...
  return<div style={{maxWidth:900}}>
    <div style={{display:"flex",justifyContent:"space-between",marginBottom:18}}><div style={{fontSize:11,color:T.dim}}>Klik antal for hurtig-edit</div><Btn primary onClick={()=>{setForm({id:uid(),name:"",unit:"stk",qty:0,min:0,cat:"Råvare",leadDays:7,supplier:"",costPer:0});setShow(true)}}>+ Tilføj</Btn></div>
